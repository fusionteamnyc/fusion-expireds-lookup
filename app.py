...
if single_submit and single_address:
    ...
    # ----- Email this result (always visible) -----
    st.markdown("##### Email this result")
    ...
    with em_col2:
        compose_email = st.button("Open email draft", key="single_email_btn")
    ...
    if compose_email:
        if not recipient_email or '@' not in recipient_email:
            ...
            lines = [f"Fusion Team - Owner Lookup Result", "", f"Property: {single_address}",
                     f"Type: {single_type}"]
            if owner:
                lines.append(f"Owner (ACRIS): {owner}")
                if result.get('owner_is_entity'):
                    lines.append("Owner Is: Business entity (LLC / Corp / Trust)")
                if result.get('owner_mailing_address'):
                    lines.append(f"Mailing Address: {result['owner_mailing_address']}")
                if result.get('deed_date'):
                    lines.append(f"Deed Date: {str(result['deed_date'])[:10]}")
                if result.get('dos_process_name'):
                    lines.append(f"Registered Agent: {result['dos_process_name']}")
                if result.get('dos_process_address'):
                    lines.append(f"Agent Address: {result['dos_process_address']}")
                if result.get('dos_status_note'):
                    lines.append(f"Entity Status: {result['dos_status_note']}")
            if result.get('is_sponsor_unit'):
                lines.append("Note: This unit appears to be sponsor-owned (no buyer deed on record yet).")
            elif 'no individual deed' in (status or ''):
                lines.append("Note: Co-op / Condop. Use a reverse-address lookup to find the resident.")
            elif not owner:
                lines.append("Note: No deed found on file. May require manual review.")
            lines.append("")
            lines.append("---")
            lines.append("Generated by the Fusion Team Owner Lookup tool.")
            
            subject = f"Fusion Team Owner Lookup - {single_address}"
            body = "\n".join(lines)
            mailto = f"mailto:{recipient_email}?subject={quote(subject)}&body={quote(body)}"
            
            # Open the user's default email client in a new tab
            st.markdown(
                f'<a href="{mailto}" target="_blank" '
                f'style="display:inline-block; padding:0.6rem 1.2rem; background:#ffffff; '
                f'color:#000000; text-decoration:none; border-radius:4px; font-weight:600; '
                f'font-size:0.85rem; letter-spacing:0.05em; text-transform:uppercase;">'
                f'Click here to open your email app</a>',
                unsafe_allow_html=True
            )
            st.caption("Your email client will open with the draft. Review and click Send.")
...
if run_batch and uploaded_file:
    ...
    want_email = st.checkbox(
        ...
        value=False,
        help="Opens your email client with a summary of this batch. Files are NOT attached "
             "(they're already in your Downloads folder)."
    )
    
    if want_email:
        em_col1, em_col2 = st.columns([3, 1])
        with em_col1:
            batch_email_to = st.text_input(
                "Send summary to",
                placeholder="recipient@example.com",
                label_visibility="collapsed",
                key="batch_email_to",
        ...
        with em_col2:
            compose_batch_email = st.button("Open email draft", key="batch_email_btn")
        
        if compose_batch_email:
            if not batch_email_to or '@' not in batch_email_to:
                st.warning("Please enter a valid email address.")
            else:
                ...
                if matches or mismatches:
                    lines.append(f"Name cross-check: {matches} verified, {mismatches} mismatches to investigate")
                lines.extend([
                    "",
                    "Output files saved to the runner's Downloads folder:",
                    f"  - {full_filename}",
                    f"  - {idi_filename}" if idi_csv else "  - (skip-trace file not generated)",
                    "",
                    "To share the files, attach them manually from Downloads.",
                    "",
                    "---",
                    "Generated by the Fusion Team Owner Lookup tool.",
                ])
                subject = f"Fusion Team Batch Complete - {n} properties - {requester}"
                body = "\n".join(lines)
                mailto = f"mailto:{batch_email_to}?subject={quote(subject)}&body={quote(body)}"
                ...
                st.caption("Your email client will open with the draft. Review and click Send. "
                           "Attach the CSV files manually from your Downloads folder.")
